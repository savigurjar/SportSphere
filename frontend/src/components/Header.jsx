...
import { 
  ...
  Home, Users, Calendar, Target, Award, 
  Settings, Bell, ChevronDown, Sparkles, 
  Info, MapPin, Phone, Mail, BookOpen, Dumbbell,
  Activity, Star, TrendingUp, Award as AwardIcon,
  Building, GraduationCap, Brain, HeartPulse
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useDispatch, useSelector } from "react-redux";
import { logoutUser } from "../features/authSlice";
    ...
    <>
      ...
      >
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden md:flex items-center gap-1.5">
            {filteredNavItems.map((item, index) => (
              ...
              >
                <Link
                  to={item.path}
                  className={`relative flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-all duration-300 group/nav ${
                    location.pathname === item.path
                      ? 'text-[#DCE7C6] bg-white/5'
                      : 'text-gray-300 hover:text-[#DCE7C6]'
                  }`}
                >
                  <item.icon className="w-4 h-4" />
                  <span>{item.label}</span>
                  ...
                  {/* Hover effect */}
                  <div className={`absolute inset-0 bg-gradient-to-r from-blue-900/0 via-blue-700/0 to-[#DCE7C6]/0 rounded-lg transition-all duration-300 ${
                    location.pathname === item.path 
                      ? 'from-blue-900/10 via-blue-700/10 to-[#DCE7C6]/10' 
                      : 'group-hover/nav:from-blue-900/10 group-hover/nav:via-blue-700/10 group-hover/nav:to-[#DCE7C6]/10'
                  }`} />
                  <div className={`absolute bottom-0 left-1/2 h-0.5 bg-gradient-to-r from-blue-400 to-[#DCE7C6] transition-all duration-300 ${
                    location.pathname === item.path 
                      ? 'w-8 -translate-x-4' 
                      : 'w-0 group-hover/nav:w-8 group-hover/nav:-translate-x-4'
                  }`} />
                </Link>
              </motion.div>
            ))}
          ...
          <div className="flex items-center gap-3">
            {/* Notification Bell */}
            <NotificationBell count={isAuthenticated ? 3 : 0} />
            ...
            <div className="hidden md:flex items-center gap-3">
              {!isAuthenticated ? (
                <>
                  ...
                  <Link
                    to="/signup"
                    className="px-4 py-2 text-sm font-medium bg-gradient-to-r from-blue-700 to-blue-900 hover:from-blue-600 hover:to-blue-800 rounded-lg text-white transition-all group/signup relative overflow-hidden"
                  >
                    <span className="relative z-10">Join HSA</span>
                    <div className="absolute inset-0 bg-gradient-to-r from-blue-600 to-blue-800 opacity-0 group-hover/signup:opacity-100 transition-opacity" />
                  </Link>
                </>
              ) : (
                <div className="relative">
